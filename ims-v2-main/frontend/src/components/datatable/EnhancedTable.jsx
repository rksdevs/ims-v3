...
// import EnhancedTable from "./sampleDataTable";
import { visuallyHidden } from '@mui/utils';import PropTypes from 'prop-types';
...
// const Datatable = ({data}) => {
  ...
  function EnhancedTableHead(props) {
    const {onSelectAllClick, order, orderBy, numSelected, rowCount, onRequestSort, rows, headCells} = props;
...
export default function EnhancedTable({data}) {
    ...
    useEffect(()=>{
      console.log(data, "from checkpoint")
      let arrayToAppend = []; 
      data?.columns?.forEach(element => {
        arrayToAppend.push(element.field);
      });
      setHeadCells(arrayToAppend);
    },[data]);

    useEffect (()=> {
      
      console.log(headCells, "setheadcells")
    }, [data, headCells])
    ...
    const visibleRows = React.useMemo(
      ...
      [data?.rows, order, orderBy, page, rowsPerPage],
    );
...
//   return ( newData && newData.length !== 0 ? (<div>here</div>) : (<div>Loading</div>)
//   )
// }
